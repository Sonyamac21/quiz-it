"use client";
// MATCH MADE — NEW ROUND TYPE, DESIGN-ONLY PREVIEW. (Formerly named "Pairs"
// during design — renamed to Match Made, the host's pick.)
//
...
type MatchTile = { id: string; pairId: string; label: string; glyph: string; bg: string };
...
function useMatchMadeDemo() {
  const [tiles] = useState<MatchTile[]>(buildTiles);
...
export function MatchMadeHostConsolePreview({ onClose }: { onClose?: () => void }) {
  const demo = useMatchMadeDemo();
  return (
    <div style={{ minHeight: "100vh", boxSizing: "border-box", background: "#0A0118", display: "flex", flexDirection: "column" }}>
      ...
      <div style={{ flexShrink: 0, background: "linear-gradient(135deg,#be26c1,#7a1a7d)", padding: "14px 24px", display: "flex", alignItems: "center", gap: 14 }}>
        ...
        <span style={{ color: "#fff", fontSize: 16, fontWeight: 800 }}>End Match Made round &amp; reveal scores</span>
      ...
      <div style={{ flexShrink: 0, display: "flex", alignItems: "center", gap: 12, padding: "14px 24px 6px" }}>
        <div style={{ fontFamily: "'Bruno Ace SC', sans-serif", fontSize: 20, color: "#D94FDC", letterSpacing: 3 }}>MATCH MADE</div>
...
export function MatchMadeDisplayPreview() {
  const demo = useMatchMadeDemo();
  return (
    <div style={{ minHeight: "100vh", background: "#0A0118", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", padding: "40px 16px" }}>
      <div style={{ color: "#cfc2e7", fontSize: 14, letterSpacing: 2, fontWeight: 700 }}>MATCH MADE</div>
...
export function MatchMadePlayerPreview() {
  const demo = useMatchMadeDemo();
  return (
    <div style={{ minHeight: "100dvh", background: "radial-gradient(ellipse 90% 50% at 50% 15%,rgba(190,38,193,.13),transparent 70%),#090116", display: "flex", flexDirection: "column", alignItems: "center", padding: "24px 16px" }}>
      <div style={{ fontFamily: "'Bruno Ace SC', sans-serif", color: "#ffc533", fontSize: 15, letterSpacing: 2, marginTop: 8 }}>MATCH MADE</div>
          ...
          return (
            <button
              ...
              style={{
                ...
                minHeight: 64,
                animation: isWrong ? "qi-match-made-shake .5s" : undefined,
      ...
      <style>{`@keyframes qi-match-made-shake { 0%,100%{transform:translateX(0)} 25%{transform:translateX(-6px)} 75%{transform:translateX(6px)} }`}</style>
    </div>
  );
}
