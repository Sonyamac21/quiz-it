-- Match Made previously awarded 1 point per solved pair (3 available per
-- question). Host has asked for 2 points per match (6 available). Never
-- edit an already-applied migration in place - this re-defines the same
-- function with the award amount doubled everywhere it's written, keeping
-- every other rule (idempotent event_key, authorisation checks, progress
-- shape) identical to 202609210001_pairs_completion.sql.
...
begin
  ...
  begin
    ...
    values(p_session_pin, v_canonical_team, 2, 2, 0, 0, now())
    on conflict(session_pin, team_name) do update set
      total_points = scores.total_points + 2,
      round_points = scores.round_points + 2,
      updated_at = now();
  exception when unique_violation then
    -- Progress and award use the same durable key. A reconnect/retry can
    -- never earn the points twice.
    null;
  end;
