...
export default function QuizBuilderPage() {
  ...
  const load = useCallback(async () => {
    ...
    if (quizError) setError("Quiz Builder migration is required. " + quizError.message);
  ...
  async function deleteQuiz(quiz: QuizDefinition) { if (!confirm(`Delete “${quiz.name}”?`)) return; await createSupabaseBrowserClient().from("quizzes").delete().eq("id", quiz.id); setSelectedId(null); await load(); }

  return <HostShell><main style={{ minHeight: "100vh", background: BG, color: "#fff", padding: "24px 32px" }}>
    <div className="fbh-top"><span className="fbh-wm"><span className="q">QUIZ-</span>IT</span><span className="fbh-bc">Quiz Builder</span><TopSpacer /><Link className="fbh-btn" href="/host/rounds">Round Library</Link><Link className="fbh-btn pri" href="/host/session">Create Session</Link></div>
    {loading ? <HostLoading title="Quiz Builder" note="Loading quizzes and rounds…" /> : error && !quizzes.length ? <div className="fbh-panel" role="alert">{error}</div> : <div className="qi-quiz-builder-grid">
      <section className="fbh-panel"><HostLabel>New Quiz</HostLabel><HostInput value={name} onChange={e => setName(e.target.value)} placeholder="Thursday Night Quiz" /><textarea value={description} onChange={e => setDescription(e.target.value)} placeholder="Optional description" rows={2} className="fbh-input" style={{ width: "100%", marginTop: 8 }} /><HostButton variant="pri" onClick={createQuiz} disabled={!name.trim() || saving} style={{ width: "100%", marginTop: 10 }}>CREATE QUIZ</HostButton>
        <div className="fbh-lbl" style={{ marginTop: 22 }}>Your Quizzes</div>{quizzes.length ? quizzes.map(q => <button key={q.id} onClick={() => setSelectedId(q.id)} className="fbh-answer-row" style={{ width: "100%", cursor: "pointer", borderColor: q.id === selectedId ? "#BE26C1" : undefined, opacity: q.archived ? .55 : 1 }}><span className="nm">{q.name}</span><span className="ans">{q.quiz_rounds.length} rounds{q.archived ? " · Archived" : ""}</span></button>) : <HostEmpty title="No Quizzes Yet" note="Create one, then add reusable rounds." />}
      </section>
      <section className="fbh-panel">{!selected ? <HostEmpty title="Select a Quiz" note="Choose a quiz to arrange its running order." /> : <>
