...
export async function checkQuestion(q: Question, theme: string, recencyNote?: string): Promise<{ ok: boolean; note: string; unavailable?: boolean }> {
  ...
  const prompt =
    ...
    "Also verify that the Answer is factually correct for the Question, using the literal reference metadata when needed. For Multi Tap, verify every option independently: every option named by the answer key must be true and every unselected option must be false. Pay special attention to geographic facts (city, country, capital, venue) and named entities - these are common error points, so check them precisely rather than assuming they are right. " +
    (recencyNote
...
export async function finalQualityCheck(q: Question, theme: string, recencyNote?: string): Promise<{ ok: boolean; note: string }> {
  ...
  const prompt =
    ...
    "For Multi Tap specifically, every wrong option must be a credible near-miss from the SAME semantic category as the correct options. Reject any joke, nonsense, giveaway, or category-mismatched decoy that no reasonable player could confuse with a correct answer. For example, 'Concrete' among types of cheese MUST FAIL; use a plausible cheese-like name or a real cheese that narrowly fails the stated condition instead. A question with five obvious correct items and one absurd outsider MUST FAIL even when its answer key is factually accurate. " +
