...
export default function EventCalendarPage() {
  ...
  const filtered=useMemo(()=>events.filter(event=>(!filters.venue||event.venue_record_id===filters.venue)&&(!filters.host||(event.host_name||event.host_id)===filters.host)&&(!filters.status||event.status===filters.status)&&(showPast||event.event_date>=localDateKey())),[events,filters,showPast]);
  ...
  function chooseVenue(id:string){const venue=venues.find(v=>v.id===id);if(!venue)return;setDraft(value=>value?{...value,venueId:id,start:venue.default_start_time?.slice(0,5)||value.start,end:venue.default_end_time?.slice(0,5)||value.end,hostId:venue.default_host_id||currentHost.id,hostName:venue.default_host_name||currentHost.name,quizId:venue.default_quiz_id||value.quizId,offers:"",sponsors:""}:value);}
  ...
  const eventButton=(event:EventRecord)=>{const missingQuiz=!event.quiz;return <button key={event.id} draggable onDragStart={e=>e.dataTransfer.setData("text/event-id",event.id)} onClick={()=>editEvent(event)} style={{width:"100%",textAlign:"left",padding:"12px",borderRadius:10,border:`1px solid ${missingQuiz?"#FFC533":hostColour(event.host_name||event.host_id)}`,background:missingQuiz?"rgba(255,197,51,0.12)":`${hostColour(event.host_name||event.host_id)}22`,color:"#fff",cursor:"pointer",marginBottom:7,minWidth:0,maxWidth:"100%",overflow:"hidden"}}><strong style={{fontSize:18,display:"block",overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{String(event.start_time).slice(0,5)} · {event.venue?.venue_name||event.event_name}</strong><span style={{display:"block",fontSize:16,marginTop:4,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{missingQuiz?<span style={{color:"#FFC533",fontWeight:700}}>⚠ No Quiz Plan</span>:<span style={{color:"#B9A8D9"}}>{event.quiz?.name}</span>}<span style={{color:"#B9A8D9"}}> · {event.status}</span></span></button>;};
  ...
  return <HostShell><main className="qi-bo-page">
    ...
    {view==="week"&&<section style={{display:"grid",gridTemplateColumns:"repeat(7,minmax(150px,1fr))",gap:8}}>{weekDates.map(date=>{const key=localDateKey(date);return <div className="fbh-panel" key={key} onDragOver={e=>e.preventDefault()} onDrop={e=>void moveEvent(e.dataTransfer.getData("text/event-id"),key)} style={{minHeight:180}}><button onClick={()=>chooseDate(key)} className="fbh-btn" style={{width:"100%",marginBottom:10}}>{date.toLocaleDateString("en-GB",{weekday:"short",day:"numeric"})}</button>{filtered.filter(e=>e.event_date===key).map(eventButton)}</div>})}</section>}
    {view==="agenda"&&<section>{filtered.length?filtered.map(event=><article key={event.id} className="fbh-panel" style={{display:"grid",gridTemplateColumns:"160px minmax(0,1fr) auto",gap:18,alignItems:"center"}}><div><strong>{formatEventDate(event.event_date)}</strong><div style={{color:"#B9A8D9"}}>{formatEventTime(event.start_time)}</div></div><div style={{minWidth:0,overflow:"hidden"}}><strong style={{display:"block",overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{event.venue?.venue_name||event.event_name}</strong><div style={{color:"#B9A8D9",marginTop:4,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{event.host_name||"Host"} · {event.quiz?<span>{event.quiz.name}</span>:<span style={{color:"#FFC533",fontWeight:700}}>⚠ No Quiz Plan</span>} · {event.status}</div></div><button className="fbh-btn" onClick={()=>editEvent(event)}>EDIT</button></article>):<div className="fbh-panel">No events match these filters.</div>}</section>}
