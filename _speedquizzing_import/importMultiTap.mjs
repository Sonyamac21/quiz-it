...
//
// Reads speedquizzing_multitap_reviewed.csv (produced by
...
const IN_CSV = path.join(__dirname, "speedquizzing_multitap_reviewed.csv");
const REPORT_PATH = path.join(__dirname, "import_report_multitap.csv");
...
async function main() {
  if (!fs.existsSync(IN_CSV)) {
    console.error(`Missing ${IN_CSV} - run reviewMultiTap.mjs first.`);
    process.exit(1);
  }
  const env = loadEnv();
  ...
  const payload = toImport.map(r => ({
    ...
    source: "speedquizzing_multitap_import",
    needs_review: true,
    stale_risk: false,
