...
export default function BackOfficeDashboard(){
  ...
  // Upcoming quizzes used to be its own full-width section below the grid -
  // on a light day (one or zero events today) that left the Today column
  // mostly empty while Upcoming sat unrelated underneath, forcing extra
  // scrolling for no reason. Stacked into the same column as Today now, so
  // the space actually gets used; excludes today's own events since those
  // are already listed right above in the Today list.
  const upcomingEvents=weekEvents.filter(e=>e.event_date!==today);
  ...
  return <main className="qi-bo-page"><header className="qi-bo-pagehead"><div><p>Back Office</p><h1>Good evening</h1><span>Everything needed to prepare the next live event.</span></div><Link className="qi-bo-primary" href="/host/events">Open Calendar</Link></header>{error&&<div className="qi-bo-alert" role="alert">Dashboard data could not be fully loaded: {error}</div>}
    ...
    <div className="qi-bo-dashboard-grid"><section><div className="qi-bo-sectionhead"><div><p>Today</p><h2>Today&apos;s quizzes</h2></div><Link href="/host/events">Full calendar</Link></div>{loading?<div className="qi-bo-card">Loading today&apos;s schedule…</div>:todayEvents.length?todayEvents.map(event=><EventRow key={event.id} event={event}/>):<div className="qi-bo-empty"><strong>No quizzes today</strong><span>The calendar is clear.</span><Link href="/host/events">Schedule an event</Link></div>}
        <div className="qi-bo-sectionhead"><div><p>This week</p><h2>Upcoming quizzes</h2></div></div>{upcomingEvents.length?upcomingEvents.map(event=><EventRow key={event.id} event={event}/>):<div className="qi-bo-empty"><strong>No upcoming quizzes</strong><span>Choose a date in Calendar to schedule one.</span></div>}
      </section>
