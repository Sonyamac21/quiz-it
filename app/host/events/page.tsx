...
export default function EventCalendarPage() {
  ...
  const eventButton=(event:EventRecord)=><button key={event.id} draggable onDragStart={e=>e.dataTransfer.setData("text/event-id",event.id)} onClick={()=>editEvent(event)} style={{width:"100%",textAlign:"left",padding:"7px 9px",borderRadius:8,border:`1px solid ${hostColour(event.host_name||event.host_id)}`,background:`${hostColour(event.host_name||event.host_id)}22`,color:"#fff",cursor:"pointer",marginBottom:5}}><strong style={{fontSize:12}}>{String(event.start_time).slice(0,5)} · {event.venue?.venue_name||event.event_name}</strong><span style={{display:"block",fontSize:10,color:"#B9A8D9",marginTop:2}}>{event.quiz?.name||"No Quiz Plan"} · {event.status}</span></button>;
